(function() {
  ...
  // add points to the game.  If its a big asteroid split it.
  // if it's a small asteroid remove it

  MovingObject.prototype.bulletvsAsteroid = function () {
    this.game.score += 100
    if (this.radius === 20) {
      this.game.remove(this)
    } else {
      var new_asteroids = this.split();
       this.game.splitAsteroid(new_asteroids)
       this.game.remove(this)
